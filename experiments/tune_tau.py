...
from src.sentinelgate import load_config, SentinelGate, Embedder


def eval_with_tau(df: pd.DataFrame, tau: float, embedder: Embedder):
    cfg = load_config()
    ...
    # ✅ Reuse shared embedder so HF model does NOT reload each sweep
...
def main():
    ...
    taus = [0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40]
    rows = [eval_with_tau(df, t, embedder) for t in taus]
